...
export default function AdFilms() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      try {
        ...
        console.error("Error fetching ad films:", error);
        setVideos([]);
        setTotalVideos(0);
      } finally {
        setIsLoading(false);
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-3xl font-semibold text-center mb-6">Our Ad Films</h1>
      <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {Array.isArray(videos) && videos.length > 0 ? (
          ...
          <div className="col-span-3 text-center">No ad films available</div>
        )}
      </div>
      <div ref={loadMoreRef} className="h-10" />
      {isLoading && (
        <div className="text-center my-4">Loading more ad films...</div>
      )}
      {searchParams.page * searchParams.limit >= totalVideos &&
        videos.length > 0 && (
          <div className="text-center my-4">No more ad films to load</div>
        )}
    </div>
  );
}
